...
|------------|---------|------------------|
| `assess-product-change` | dispatch only (via `/buddy:start`) | 기존 프로덕트 변경 평가 — 버그/기능/개선 무관하게 change trigger 접수 → 영향 평가 → scope 분류(small/medium/large) → 다음 phase routing. `concretize-idea`가 "없는 것을 만든다"면 이 스킬은 "있는 것을 바꾼다" |
