...
**진입 조건**: Production traffic 존재 (7단계 GA release 완료).
**산출물**: Experiment results, actor별 funnel analysis, improvement backlog (→ 2단계 재진입).
**다음 phase**: 분석 결과 기반 개선 → `define-features` (2단계) 재진입.

> **Q3=(c) 우선순위**: 8단계은 현재 buddy의 최대 약점 영역. 본 orchestrator를 통해 A/B 실험, funnel 분석, 인시던트 대응, 고객 피드백 처리를 지원한다.
...
```
iterate-product (8단계 phase orchestrator)
├── [Observation loop]
...
└── → define-features (2단계 재진입) — 개선 backlog 처리
```
...
2단계 use case 분해 결과를 기반으로 actor별 funnel을 추적한다:
```
...
4. Fix 배포 (`ship-release` 7-3 단계 fast path)
5. 고객 커뮤니케이션
...
improvement_task:
  ...
  hypothesis: {개선 가설}
  priority: RICE score
  next_action: define-features (2단계) → {feature spec draft}
```
...
다음 조건에서 `define-features` (2단계)로 재진입한다:
