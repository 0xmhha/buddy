...
purge + 페르소나 카탈로그 + 4 friction fix.
- 15개 패키지, ~150+ tests pass (race-clean), ~9.5MB static binary.
- 다음: M6 release prep 진행 중 (cross-compile matrix + tag-triggered GH
  Actions release + CHANGELOG).
...
`gh` CLI 사용자라면 release asset를 바로 가져올 수 있습니다:

```bash
gh release download v0.1.0 --repo 0xmhha/buddy \
  --pattern "buddy_*_$(go env GOOS)_$(go env GOARCH)" -O buddy
```
