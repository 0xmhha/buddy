...
| W3-2 TUI | bubbletea 학습 + agent list / create form | HIGH (partial Done 2026-05-13 — minimum-viable: bubbletea + lipgloss deps + `buddy tui` subcommand + read-only agent list view with j/k navigation, g/G jump, r refresh, q quit, AltScreen lifecycle. **2026-05-15 follow-on**: inline detail pane (Enter/l opens, Esc/h returns, r refetches LatestRun) renders agent metadata + latest-run summary with friend-tone empty/error copy. **2026-05-17 follow-on**: scheduler-preview pane (`s` opens) shows next-fire time per scheduled agent via `agent.PreviewSchedule` helper (cron.ParseStandard) — preview only, decoupled from running scheduler. **2026-05-17 follow-on**: in-app delete with confirm pane (`d` → y/N prompt → `Store.Delete` + FK cascade for runs/logs) — nav keys inert in confirm mode, failure surfaces as wrapped err in list pane. **2026-05-17 follow-on**: live log-tail pane (`t` from detail) — `Store.LogsSince` incremental polling every ~1s via `tea.Tick`; self-cancels on mode change; appends oldest-first by id. **2026-05-17 follow-on**: in-app spec edit (`e` from detail) — `tea.ExecProcess` shell-out to `$EDITOR`/`$VISUAL`/`vi` on a temp YAML, then `ParseSpec` + rename guard + `Store.UpdateSpec`. Failure surfaces as `edit error: …` banner in detail pane. **Create form remains the only W3-2 follow-on item still open**) | W3-1 |
