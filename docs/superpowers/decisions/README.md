...
| [ADR-020](./2026-06-02-lazy-load-architecture.md) | 2026-06-02 | Lazy-Load Architecture: 4-Layer skill loading (router auto-load + phase catalog + procedure frontmatter line-range + body full). Token cost ~10.7K chars max vs ~19K chars baseline; phase isolation prevents similar-pattern skill conflict (e.g. ideation brainstorming vs bug-fix analysis); enables Anthropic-standard frontmatter on procedure files while keeping cost low via name-based exclusion from auto-listing. Supersedes catalog-only dispatch. | Accepted | architecture, lazy-load, token-cost, skill-conflict-prevention, frontmatter, catalog-split, lifecycle-isolation |
...
| ~~ADR-{N} F2.D Drift Detection design~~ | **Closed by ADR-017 (2026-05-20)** — embedding cosine + KindGoalDrift advisor rule + no new surface |
