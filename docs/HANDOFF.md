...
**Last updated:** 2026-05-12 (**v0.6.4** — Tier 1.5 streaming log capture (per-line `agent_logs` entries as the child process emits them; `buddy agent log <id>` shows mid-progress) + verify-quality-driven cleanup of the streaming-path internals (F1–F4: pipe-close hygiene, error-path FD leak fix, `translateExitCode` signature, dead-code prune). Strictly additive at the user level. **세션 인계 진입점: [`docs/notes/2026-05-11-cycle-handoff.md`](./notes/2026-05-11-cycle-handoff.md)**)
...
| **v0.6.4 patch** (Tier 1.5 streaming log capture: `Executor.Run` signature + `LogSink` + per-line `agent_logs` entries; `buddy agent log <id>` mid-progress visibility. Plus verify-quality F1–F4 cleanup: pipe-close hygiene, error-path FD leak fix, `translateExitCode` signature trim, dead-code prune) | ✅ Done — **본 publish 의 baseline** | **v0.6.4** (2026-05-12) | — |
| **Total** | **148 procedures / 99 commands / 14 ship-release stages / 7 MCP analytics tools / 7 cli agent subcommands / PROCEDURE parser + conditional branches + drift gates + exponential backoff + streaming logs** | **v0.6.4** (2026-05-12) | 148 |
