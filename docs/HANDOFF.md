...
**Last updated:** 2026-05-17 (**v0.7.0** — Bundle: 4 W3-2 follow-on items (minimum-viable list / detail view / scheduler-preview pane / in-app delete with confirm) + F5 log retention (`buddy agent purge`) + W3-6 reference webtoon agent example. Strictly additive. cli-buddy-spec §9 W3-2 row now reads "minimum-viable + 3 follow-on Done"; W3-2 still partial (create form, live log tail, in-app edit deferred). **세션 인계 진입점: [`docs/notes/2026-05-11-cycle-handoff.md`](./notes/2026-05-11-cycle-handoff.md) (oldest baseline)** — v0.7.0 cycle 인계는 `CHANGELOG.md [0.7.0]` + 본 문서 §1 표.)
...
| **cli buddy** — TUI 자동화 agent 관리 툴 (plugin buddy 내재화). 진짜 목적은 *agent 생성 / 실행 / 종료 / 설정 관리*. v0.1.0 = hook reliability monitor (한 sub-feature). | 🟢 본격 진입 — W3-1 spec Accepted (ADR-005). **W3-3 runtime + scheduler Done + W3-4 parser + conditional branches (v0.6.0) + `buddy agent log` viewer (v0.6.2) + W3-2 minimum-viable TUI + 3 follow-on (detail / scheduler-preview / in-app delete) shipped in v0.7.0 bundle 2026-05-17** — `internal/agent/` (types/spec/store w/ LatestRun/executor/runtime/scheduler/parser w/ branches/PreviewSchedule) + migration v4 + `buddy agent {create,list,show,run,log,delete,scheduler,purge}` + `buddy tui` (list/detail/scheduler/delete). W3-2 create form / live log tail / in-app edit / W3-4 retry-on-fail semantics + auto-cascade / W3-5 재배치 / W3-6 example shipped (2026-05-13) but cascade-dogfood 미구현 | `cmd/`, `internal/`, `archive/ts-poc/` | [`docs/two-tracks-charter.md`](./two-tracks-charter.md) §3 + [`docs/cli-buddy-spec.md`](./cli-buddy-spec.md) (Accepted) |
...
| **v0.7.0 release** (Bundle: W3-2 minimum-viable TUI + 3 follow-on (detail / scheduler-preview / in-app delete with confirm) + F5 log retention (`buddy agent purge --before <dur> [--apply]`) + W3-6 reference webtoon agent example. New: `agent.PreviewSchedule` helper, `internal/tui` package 51 race-clean tests) | ✅ Done — **본 publish 의 baseline** | **v0.7.0** (2026-05-17) | — |
| **Total** | **148 procedures / 99 commands / 15 ship-release stages / 7 MCP analytics tools / 8 cli agent subcommands / `buddy tui` (4 modes) / PROCEDURE parser + conditional branches + drift gates + exponential backoff + streaming logs + scheduler live refresh + webhook output + log retention + scheduler preview + in-app delete** | **v0.7.0** (2026-05-17) | 148 |
