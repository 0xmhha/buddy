...
| 트랙 | 상태 | 위치 | Entry doc |
|------|------|------|----------|
| **plugin buddy** — Claude Code plugin (skill / MCP / agent / hook 카탈로그). 9-phase orchestrator, **148 procedures, 99 commands**, single-router dispatch | 🟢 ACTIVE — Skill Completion Cycle 100% (44/44 신규 + 통합 2). charter scope 12 stage 100% cover. analytics-mcp ✅ shipped v0.3.0. 다음 후보: B-2 production dogfood (cycle-3 active) + Korea cluster trigger 시 작성 | `plugin/`, `docs/superpowers/` | [`docs/two-tracks-charter.md`](./two-tracks-charter.md) §2 + [`docs/superpowers/specs/2026-05-06-lifecycle-orchestrator-architecture.md`](./superpowers/specs/2026-05-06-lifecycle-orchestrator-architecture.md) |
| **cli buddy** — TUI 자동화 agent 관리 툴 (plugin buddy 내재화 + hook reliability monitor 통합 + AI-usage coaching 5 surface). 진짜 목적은 *agent 생성 / 실행 / 종료 / 설정 관리 + AI 사용 패턴 가시화*. | 🟢 **W3 (cascade) + Wave 7 (AI-usage coaching) 모두 Done (2026-05-19~2026-05-20)** — W3-1~W3-6 cascade (ADR-005 spec / TUI 7 modes / agent runtime / scheduler / cascade chain) + **W4~W8 = F2.A~F2.E** (v0.8.0 session / v0.9.0 usage / v0.10.0~v0.11.0 knowledge+advise / v0.12.0 notify / v0.13.0 drift detection; ADR-012~017). 추가: Wave 2 i18n sweep 100% + Wave 3 release polish AI 가능 100% (VERSION SSoT + Actions SHA pin). `internal/{agent,sessions,usage,knowledge,advisor,notify}/` + migration v8 + `buddy {agent,session,usage,knowledge,advise,notify} ...` + `buddy tui` 7 modes. **Remaining work = v1.0.0 entry B-2 (production dogfood, user-paced — cycle-3 active) 만**. | `cmd/`, `internal/`, `archive/ts-poc/` | [`docs/two-tracks-charter.md`](./two-tracks-charter.md) §3 + [`docs/cli-buddy-spec.md`](./cli-buddy-spec.md) (Accepted) |

**plugin buddy 진행 상태 (2026-05-11):**

> **Version note**: 아래 Phase 컬럼의 `v1.0.x` ~ `v1.0.8` 는 *역사적 작업 단위* — 어느 publish 도 안 됐고 현재 marketplace.json 은 **v0.13.0** (ADR-004 reset + ADR-011 milestone-driven cadence: v0.2.0 → v0.3.0 → v0.4.0 ... → v0.13.0). 각 Phase 의 commit 은 git history 에 있으나 *v1.x 시점에는 release tag 0건* — v0.x 시리즈에서 published.
...
| **v0.7.2 patch** (TUI hook-stats pane, A-3.2 W3-5 follow-on: `H` in list view → ModeHookStats pane via `internal/queries.Run` (count/failures/p50/p95 per hook, default window 1h, same column ordering as `buddy stats` CLI). cli-buddy-spec §9 W3-5 fully closed — main.go split (v0.6.3) + hook monitor 통합 (v0.7.2). 95 race-clean TUI tests.) | ✅ Done | v0.7.2 (2026-05-18) | — |
...
| **ADR-008** (feature-management-mcp scope lock-in — 5 MCP tools + features table v0.3.0 shape) | ✅ Done | (2026-05-19) | — |
| **ADR-009** (cli buddy vision expansion — 9 책임 영역, F2.A~F2.E AI-usage coaching 추가) | ✅ Done | (2026-05-19) — Wave 7 (W4~W8) framing 도입 | — |
| **ADR-010** (v1.0.0 whole-product scope — B-1~B-4 + C-1~C-5 = 9 조건 framework) | ✅ Done | (2026-05-19) — supersedes ADR-004 의 plugin-only framing | — |
| **ADR-011** (release policy milestone-driven — Wave 완료 단위로 minor bump) | ✅ Done | (2026-05-19) | — |
| **v0.8.0 release / ADR-012** (W7-1 F2.A Session Monitor — hybrid hook+fsLister + schema v5 + CLI `buddy session {list,show,purge}` + daemon poll. closes **C-1**) | ✅ Done | v0.8.0 (2026-05-19) | — |
| **v0.9.0 release / ADR-013** (W7-2 F2.B Usage Analysis — `internal/usage/` 7 metric + CLI `buddy usage {today,trend,top}` + 5 MCP `usage_query_*` + TUI Usage pane. closes **C-2**) | ✅ Done | v0.9.0 (2026-05-19) | — |
| **v0.10.0 release / ADR-014 Phase 1/3** (W7-3a F2.C Advisory foundation — BM25 + optional embeddings + CLI `buddy knowledge {ingest,query,stats}` + MCP `knowledge_query`) | ✅ Done | v0.10.0 (2026-05-19) | — |
| **v0.11.0 release / ADR-015 Phase 2/3** (W7-3b F2.C Advisory phase 2 — `buddy advise` 5 rules + history/mute + daemon advisor monitor + TUI advise pane + MCP `usage_advise`. closes **C-3**) | ✅ Done | v0.11.0 (2026-05-19) | — |
| **v0.12.0 release / ADR-016** (W7-5 F2.E Notification — 4 channel (desktop / webhook / TUI banner / shell) + daemon auto-dispatch + per-channel severity floor + dedup + notification_log v8. closes **C-5**) | ✅ Done | v0.12.0 (2026-05-20) | — |
| **v0.13.0 release / ADR-017** (W7-4 F2.D Drift Detection — cosine over embedding + KindGoalDrift advisor rule, no new surface, 3 config keys. closes **C-4 — final C-x**) | ✅ Done — **본 publish 의 baseline** | **v0.13.0** (2026-05-20) | — |
| **Wave 2 i18n sweep** (W2-1 ValidationError persona / W2-2 queries sentinels persona / W2-3 en parity / W2-4 locale resolution from subcommand --config) | ✅ Done | (2026-05-20, post-v0.13.0) | — |
| **Wave 3 release polish (AI 가능 부분)** (W3-1 VERSION file SSoT + `make set-version` / W3-2 Actions SHA pin + Dependabot. W3-4 macOS notarization 만 Apple Dev ID trigger-bound) | ✅ Done | (2026-05-20, post-v0.13.0) | — |
| **Total** | **148 procedures / 99 commands / 27 ship-release stages + 12 ADRs / 7 MCP analytics + 5 usage_query MCP + 5 feature MCP + 1 knowledge_query + 1 usage_advise + 1 notify_send / 8 cli agent subcommands + 5 W7 subcommands (session/usage/knowledge/advise/notify) + `buddy tui` (7 modes) / advisor 5 rules + drift detection + 4-channel notify / Wave 2 i18n sweep 100% / Wave 3 release polish AI 가능 부분 100% (VERSION SSoT + Actions SHA pin + Dependabot)** | **v0.13.0 + ADR-006~017** (2026-05-20) | 148 |
| **Plugin v1.0.0 entry status (ADR-010 framework, 9 조건)** | B-1 W3 cascade ✅ / B-2 production dogfood ⚠ user-paced (**cycle-3 active**) / B-3 ✅ ADR-006 / B-4 ✅ ADR-007 // C-1 ✅ v0.8.0 / C-2 ✅ v0.9.0 / C-3 ✅ v0.11.0 / C-4 ✅ v0.13.0 / C-5 ✅ v0.12.0 — **8/9 closed, B-2 만 잔여** | — | — |
| Korea cluster 3 (consult-korea-legal-context / draft-korea-patent-application / audit-korea-cii-vulnerability) | ⏳ deferred (Wave 5) | trigger: target market = Korea | — |
| ~~analytics-mcp~~ | ✅ **Shipped v0.3.0 (2026-05-11)** — 7 MCP tools (analytics_query_*) + SQLite reference adapter. trigger 충족 후 ship 완료. | — | — |
...
**Dogfood guide (v0.13.0 — Plugin v1.0.0 entry condition B-2 실행 절차):** [`docs/dogfood-guide.md`](./dogfood-guide.md) — 3 surface (plugin / cli buddy / hook monitor) 통합 실행 + finding 수집 format + improvement workflow + ADR escalation. **현 active cycle**: [`docs/notes/2026-05-20-dogfood-result-cycle-3.md`](./notes/2026-05-20-dogfood-result-cycle-3.md) (§A pre-flight + §B.4 W7 surface smoke + §B.1 plugin Stage 1 완료, §B.1 Stage 2-8 / §B.2 cli-buddy / §B.3 hook-monitor 진행 대기). v0.1 hook-only legacy 는 [`DOGFOOD.md`](../DOGFOOD.md) (deprecated entry, hook 부분만 유효).
...
| `docs/notes/2026-05-20-dogfood-result-cycle-3.md` | **현 active dogfood cycle** (B-2 충족 시도, v0.13.0 baseline, §A + §B.1 Stage 1 + §B.4 5/5 surface 완료) |
| `docs/notes/2026-05-11-cycle-handoff.md` | 이전 cycle close 핸드오프 (v0.2.0 reset → v0.3.0 ship → W3-3 partial). 역사적 reference |
