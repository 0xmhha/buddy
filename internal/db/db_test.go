...
func TestOpen_CreatesAllTables(t *testing.T) {
	...
	for _, want := range []string{"hook_outbox", "hook_events", "hook_stats", "features", "sessions", "agents", "agent_runs", "agent_logs", "chunks", "schema_version"} {
...
func TestOpen_RecordsLatestSchemaVersion(t *testing.T) {
	path := filepath.Join(t.TempDir(), "buddy.db")
	conn, err := db.Open(db.Options{Path: path})
	require.NoError(t, err)
	t.Cleanup(func() { _ = conn.Close() })
...
func TestOpen_IsIdempotentAcrossReopens(t *testing.T) {
	...
	assert.Equal(t, 6, count) // one row per applied migration
}
