...
func TestOpen_CreatesAllTables(t *testing.T) {
	...
	for _, want := range []string{"hook_outbox", "hook_events", "hook_stats", "features", "sessions", "agents", "agent_runs", "agent_logs", "schema_version"} {
...
func TestOpen_IsIdempotentAcrossReopens(t *testing.T) {
	...
	assert.Equal(t, 5, count) // one row per applied migration
}
