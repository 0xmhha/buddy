...
func TestStore_List_IncludeMutedReturnsAll(t *testing.T) {
	...
	ctx := context.Background()
	now := time.Now().UTC().Truncate(time.Millisecond)
	store.Insert(ctx, sampleAdvisory(KindTokenSpikeDay, SeverityWarn, now))
	id2, _ := store.Insert(ctx, sampleAdvisory(KindLongSession, SeverityHigh, now))
	require.NoError(t, store.Mute(ctx, id2))
...
func TestStore_List_OrderedDesc(t *testing.T) {
	...
	ctx := context.Background()
	now := time.Now().UTC().Truncate(time.Millisecond)
	store.Insert(ctx, sampleAdvisory("a", SeverityInfo, now.Add(-2*time.Hour)))
	store.Insert(ctx, sampleAdvisory("b", SeverityInfo, now))
	store.Insert(ctx, sampleAdvisory("c", SeverityInfo, now.Add(-1*time.Hour)))
...
func TestStore_List_SinceFilter(t *testing.T) {
	...
	ctx := context.Background()
	now := time.Now().UTC().Truncate(time.Millisecond)
	store.Insert(ctx, sampleAdvisory("old", SeverityInfo, now.Add(-48*time.Hour)))
	store.Insert(ctx, sampleAdvisory("recent", SeverityInfo, now))
...
func TestStore_List_KindsFilter(t *testing.T) {
	...
	ctx := context.Background()
	now := time.Now().UTC().Truncate(time.Millisecond)
	store.Insert(ctx, sampleAdvisory(KindTokenSpikeDay, SeverityInfo, now))
	store.Insert(ctx, sampleAdvisory(KindLongSession, SeverityInfo, now))
	store.Insert(ctx, sampleAdvisory(KindLowCacheRatio, SeverityInfo, now))
...
func TestStore_LastInWindow_FindsRecent(t *testing.T) {
	...
	ctx := context.Background()
	now := time.Now().UTC().Truncate(time.Millisecond)
	store.Insert(ctx, sampleAdvisory(KindTokenSpikeDay, SeverityWarn, now.Add(-30*time.Minute)))
...
func TestStore_LastInWindow_TooOldReturnsNotFound(t *testing.T) {
	...
	ctx := context.Background()
	now := time.Now().UTC().Truncate(time.Millisecond)
	store.Insert(ctx, sampleAdvisory(KindTokenSpikeDay, SeverityWarn, now.Add(-25*time.Hour)))
...
func TestStore_MuteKind(t *testing.T) {
	...
	store.Insert(ctx, sampleAdvisory(KindTokenSpikeDay, SeverityWarn, now))
	store.Insert(ctx, sampleAdvisory(KindTokenSpikeDay, SeverityWarn, now))
	store.Insert(ctx, sampleAdvisory(KindLongSession, SeverityHigh, now))
